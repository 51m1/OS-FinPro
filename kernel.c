...
uint8_t alt_color = 0x82; // k background, e foreground
uint8_t coltst = 0x00;
...
void term_init()
{
	...
	for (int col = 0; col < VGA_COLS; col ++)
	{
		for (int row = 0; row < VGA_ROWS; row ++)
		{
			...
			vga_buffer[index] = ((uint16_t)term_color << 8) | ' '; // Set the character to blank (a space character) deff=8
		}
	}
}
...
void colprint(char c)
{
	...
	default: // Normal characters just get displayed and then increment the column
		{
			const size_t index = (VGA_COLS * term_row) + term_col; // Like before, calculate the buffer index
			vga_buffer[index] = ((uint16_t)alt_color << 8) | c;
			term_col ++;
			break;
		}
	}
